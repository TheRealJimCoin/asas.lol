...
//const connect_1 = require("@agoralabs-sh/avm-web-provider");
import connect_1 from "@agoralabs-sh/avm-web-provider";
const KIBISIS_AVM_WEB_PROVIDER_ID = "f6d1c86b-4493-42fb-b88d-a62407b4cdf6"
...
class KibisisConnectWallet {
    constructor() {
        this.accounts = [];
        this.defaultAccount = 0;
        this.kibisisConnect = new connect_1.AVMWebClient.init();
    }
    static displayName() {
        return "Kibisis Connect";
    }
    displayName() {
        return KibisisConnectWallet.displayName();
    }
    static img(inverted) {
        return inverted ? logoInverted : logo;
    }
    img(inverted) {
        return KibisisConnectWallet.img(inverted);
    }
    async _getGenesisHash() {
        //change this for VOI network
        const algodClient = new algosdk_1.Algodv2("", "https://go.getblock.io/30d783c3c1ae42ff8756c49f03f3206d", "")
        //const algodClient = new algosdk_1.Algodv2("", "https://mainnet-api.voi.nodely.dev", "")
    ...
    processTxns( txnGroup, indexesToSign = []) {
        ...
        txnGroup.forEach((txn, index) => {
          ...
          const signer = algosdk_1.encodeAddress(txn.from.publicKey)
    ...
    processEncodedTxns(txnGroup, indexesToSign = []) {
        ...
        txnGroup.forEach((txnBuffer, index) => {
          const txnDecodeObj = algosdk_1.decodeObj(txnBuffer)
    
          const isSigned = isSignedTxn(txnDecodeObj)
    
          const txn = isSigned  ? algosdk_1.decodeSignedTransaction(txnBuffer).txn : algosdk_1.decodeUnsignedTransaction(txnBuffer)
    ...
    */
    ...
    async _enable() {
        ...
        const avmWebClient = this.kibisisConnect
        ...
        return new Promise((resolve, reject) => {
            ...
            const listenerId = avmWebClient.onEnable(({ error, method, result }) => {
                ...
                console.log('Enable successful', { result })
                return resolve(result)
            })
        
            console.log('Sending enable request...', { genesisHash })
            avmWebClient.enable({
                genesisHash,
                providerId: KIBISIS_AVM_WEB_PROVIDER_ID
            })
    ...
    connect() {
        return __awaiter(this, void 0, void 0, function* () {
            if (yield this.isConnected())
                return true;
            try {
                const result = yield this._enable()
                const walletAccounts = this._mapAVMWebProviderAccountToWalletAccounts(result.accounts)
                this.accounts = walletAccounts.map((a) => { return a.address; });
                //console.log('this.accounts', this.accounts)
            }
            catch (err) {
                alert("Couldn't connect to kibisis, is it installed?");
            }
            return true;
        });
    }
    ...
    disconnect(callback) {
        if (this.kibisisConnect) {
            if (callback && typeof callback === 'function') {
                this.kibisisConnect.disconnect(callback);
            } else {
                //this.kibisisConnect.disconnect();
            }
        }
        this.accounts = [];
    }
    ...
    signTxn(txns) {
            ...
            return new Promise((resolve, reject) => {
                const timerId = window.setTimeout(() => {
                  ...
                  const listenerId = avmWebClient.onSignTransactions(({ error, method, result }) => {
                    ...
                    console.log('Sign transactions successful', result)
                    return resolve(result)
                  })
                   
                // Determine type and process transactions for signing
                //if (isTransactionArray(txns)) {
                    //const flatTxns  = this.flattenTxnGroup(txns)
                    //txnsToSign = this.processTxns(flatTxns, [2])
                //} else {
                    //const flatTxns = flattenTxnGroup(txns)
                ...
                */
                console.log('Sending sign transactions request...', txns )  
                const txnsToSign = txns.map((txn) => {
                    const txnString = this.byteArrayToBase64(txn.toByte())
                    if (algosdk_1.encodeAddress(txn.from.publicKey) !== defaultAddress)
                        return { txn: txnString };
                    return { txn: txnString };
                });

                console.log('Sending txnsToSign...', txnsToSign ) 
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const result = yield this.kibisisConnect.signTransaction([
                txnsToSign,
            ]);
...
//exports.default = KibisisConnectWallet;
